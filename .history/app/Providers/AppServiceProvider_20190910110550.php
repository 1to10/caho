...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        $baseurl = env('APP_URL').'/';
        echo $fullurl = str_replace($baseurl,'',url()->current());

        $menuifo = Menu::select('title','alias','parent_id','url','img','id')
                    ->where([['menutype','=','header'],['status','=',1]])
                    ->where([['alias','=',$fullurl]])
                    ->orderBy('ordering','ASC')->get()->first();
