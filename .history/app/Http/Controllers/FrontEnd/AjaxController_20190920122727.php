...
class Ajaxcontroller extends Controller
{
    ...
    {
        $membershipid=3;
        $artlist = MembershipRegistrations::where('membership_id','=',$membershipid)->get();
        print_r($artlist);
        foreach($artlist as $singleinfo)
        {
            print_r($singleinfo);
            echo $id = $singleinfo['id'];die;
            $MembershipValidity = date('Y-m-d',strtotime($singleinfo['MembershipValidity']));
            MembershipRegistrations::where('id', $id)->update(array('MemberValidity' => $MembershipValidity));
