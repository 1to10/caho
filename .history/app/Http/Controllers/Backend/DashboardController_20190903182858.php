...
class DashboardController extends Controller
{
    public function index()
	{
      ...
      $inactiveuserreg = Registrations::select('title','status')
							 ->where([['status','=',1]])
							 ->whereNotIn('program_id', $finalids)
							 ->count();
		
		return view('dashboard.home',compact('categories','programs','activeuserreg','inactiveuserreg'));
	}
	...
	{
		return view('dashboard.chart',compact('categories','programs','activeuserreg','inactiveuserreg'));
	}
