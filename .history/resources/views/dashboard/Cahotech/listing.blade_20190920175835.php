  ...
  <div class="box box-default">
    <div class="box-header with-border flex">
      ...
      <div class="box-tools pull-right">
        <a href="{{ env('APP_URL') }}/dashboard/cahotech/add/new"><button class="btn-success big-btn" title="Add registrations">+ Create New
            <!-- <i class="ace-icon fa fa-plus icon-only bigger-100"></i> -->
          </button></a>
        <a href="javascript:act_rec('{{ env('APP_URL') }}/dashboard/cahotech');"><button class="btn-info big-btn excell-icon" title="Publish">Download
            <!-- <i class="ace-icon fa fa-eye icon-only bigger-100"></i>  -->
          </button></a>

          <!-- <a href="javascript:act_rec('{{ env('APP_URL') }}/dashboard/cahotech');"><button class="btn-info big-btn" title="Publish">Publish
           
          </button></a>
        <a href="javascript:inact_rec('{{ env('APP_URL') }}/dashboard/cahotech');"><button class="btn-warning big-btn" title="Unpublish">Unpublish
           
          </button></a> -->
        <a href="javascript:del_rec('{{ env('APP_URL') }}/dashboard/cahotech');"><button class="btn-danger big-btn" title="Delete">Delete
                  ...
                  <tbody>
                    ...
                    <tr class="odd gradeX">
                      ...
                      <td class="center flex">
                        ...
                        <!-- @if($article->status == 1)
                        <a id="{{  $article->id }} active" class="pub_unpub" href="{{ env('APP_URL') }}/dashboard/cahotech/{{$article->id}}/deactivate" onclick="return confirm('Are you sure you want to Inactivate record!');"><img src="{{asset('backend/assets/images/tick-circle.gif')}}" width="16" height="16" border="0" title="Make Active" /></a>
                        @else
                        <a id="{{  $article->id }} inactive" class="pub_unpub" href="{{ env('APP_URL') }}/dashboard/cahotech/{{$article->id}}/activate" onclick="return confirm('Are you sure you want to Activate record!');"><img src="{{asset('backend/assets/images/unpublish.gif')}}" width="16" height="16" border="0" title="Make Inactive" /></a>
                        @endif -->

                        

                        <a href="{{ env('APP_URL') }}/dashboard/cahotech/{{ $article->id }}/destroy" onClick="return confirm('Are you sure you want to delete record!');"><i class="fa fa-trash-o fa-fw"></i></a>

                      </td>
                      <td class="fixed-sec">
                        <a href="{{ env('APP_URL') }}/dashboard/cahotech/edit/{{ $article->id }}">View</a>
                      </td>
                    </tr>
                    @endforeach
