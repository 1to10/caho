            ...
            {{ csrf_field() }}
                ...
                <table id="example1" class="table table-bordered table-striped">
                    ...
                    @foreach($articles as $article)
                        <tr class="odd gradeX">
                            <td><a href="{{ env('APP_URL') }}/dashboard/registrations?program={{ $article->id }}" target="_blank">{{  $article->title }}</a></td>
                            <td ><a href="{{ env('APP_URL') }}/dashboard/registrations?cat_id={{ $article->cat_id }}" target="_blank">{{  $article->cat_id }}</a> </td>
                            <td> <a href="{{ env('APP_URL') }}/dashboard/registrations?location={{ $article->location }}" target="_blank"> {{  $article->location }}</a></td>
