  ...
  <div class="row flex">
    ...
    <div class="col-lg-3 col-xs-6 programCat">
      ...
      <div class="small-box bg-{{$color[$index]}}">
        <div class="inner">
          <h3 style="font-size:26px;">{{$article->cat_slug}}</h3>
          <p>{{$article->title}}</p>
        </div>
        ...
        <ul class="locationlist">
          ...
          @endphp
          <li><a href="{{ env('APP_URL') }}/dashboard/registrations?cat_id={{ $article->cat_slug }}&location={{$singlecat->location}}">{{$singlecat->location}} <b>({{$usercnt}})</b></a></li>
          @endforeach
        </ul>
