...
<section class="content-header">
  <h1>
    {{$planname}} - <em>Registrations Management</em>
  </h1>
  <ol class="breadcrumb">
  ...
  <div class="box box-default">
    <div class="box-header with-border flex">
      ...
      <div class="box-tools pull-right">
        <a href="{{ env('APP_URL') }}/dashboard/member-registrations/{{$membershipid}}/add/new"><button class="btn-success big-btn" title="Add registrations">+ Create New
            <!-- <i class="ace-icon fa fa-plus icon-only bigger-100"></i> -->
          </button></a>
        <a href="javascript:act_rec('{{ env('APP_URL') }}/dashboard/member-registrations/{{$membershipid}}');"><button class="btn-info big-btn excell-icon" title="Publish">Download
            <!-- <i class="ace-icon fa fa-eye icon-only bigger-100"></i>  -->
          </button></a>

          <!-- <a href="javascript:act_rec('{{ env('APP_URL') }}/dashboard/member-registrations');"><button class="btn-info big-btn" title="Publish">Publish -->
            <!-- <i class="ace-icon fa fa-eye icon-only bigger-100"></i>  -->
          </button></a>
        <!-- <a href="javascript:inact_rec('{{ env('APP_URL') }}/dashboard/member-registrations');"><button class="btn-warning big-btn" title="Unpublish">Unpublish -->
            <!-- <i class="ace-icon fa   fa-eye-slash  icon-only bigger-100"></i> -->
          </button></a>
        <a href="javascript:del_rec('{{ env('APP_URL') }}/dashboard/member-registrations/{{$membershipid}}');"><button class="btn-danger big-btn" title="Delete">Delete
              ...
              <form>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-6 cat">
                    <input type="date" name="fromdate" placeholder="From date" value="{{$fromdate}}">
                  </div>
                  <div class="col-sm-6 cat">
                    ...
                    <select name="classification">
                      ...
                      @foreach($categorylist as $singlecity)
                        @if($singlecity->classification)
                          <option value="{{$singlecity->classification}}" @if($classification==$singlecity->classification) selected @endif>{{$singlecity->classification}}</option>
                        @endif 
                      @endforeach
                    </select>
                  </div>
                    ...
                    <select name="Organization">
                      ...
                      @foreach($program_locationlist as $singlecity)
                       @if($singlecity->organization_name)
                         <option value="{{$singlecity->organization_name}}" @if($Organization==$singlecity->organization_name) selected @endif>{{$singlecity->organization_name}}</option>
                       @endif 
                      @endforeach
                    </select>
                    </div>
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    <tr class="odd gradeX">
                      ...
                      <td>{{ date('d M, Y',strtotime($article->MembershipValidity)) }}</td>
                      <td class="center flex">


                        <input name="chkid[{{  $article->id }}]" id="chkid" type="checkbox" value="{{  $article->id }}" class="checkbtn" />

                        @if((strtotime($article->MemberValidity)) >= (strtotime(date('Y-m-d'))))
                        ...
                        <!-- <a href="{{ env('APP_URL') }}/dashboard/member-registrations/edit/{{ $article->id }}"><img src="{{asset('backend/assets/images/pencil.gif')}}" width="16" height="16" border="0" /></a> -->
                      ...
                      <td class="fixed-sec">
                        <a href="{{ env('APP_URL') }}/dashboard/member-registrations/{{$membershipid}}/edit/{{ $article->id }}">View</a>
                      </td>
                    </tr>
                    @endforeach
